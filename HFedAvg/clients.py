...
class ClientsGroup(object):
    ...
    def dataSetBalanceAllocation(self):
        ...
        # 加载测试数据
        self.test_data_loader = DataLoader(TensorDataset(test_data, test_label), batch_size=1, shuffle=False)
        ...
        sample_lists = [i for i in range(train_data.shape[0])]
        for i in range(self.num_of_clients):
            ...
            random.seed(i * 3)
            sample_list = random.sample(sample_lists, sample_num)
            #print(sample_list)
            local_data = train_data[sample_list,:]
            local_label = train_label[sample_list]
...
if __name__=="__main__":
    MyClients = ClientsGroup('breast_cancer', True, 10, 0)
    print(client)
    print(MyClients.clients_set['client5'].train_ds[0:10])
    train_ids = MyClients.clients_set['client5'].train_ds[0:10]
    i = 0
    for x_train in train_ids[0]:
        print("client5 数据:"+str(i))
        print(x_train)
        i = i+1
    print(MyClients.clients_set['client6'].train_ds[400:500])

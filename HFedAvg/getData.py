...
class GetDataSet(object):
    def __init__(self, dataSetName, isIID):
        ...
        if self.name == 'mnist':
            ...
            self.load_breastcancer(isIID)
        elif self.name == 'heart_cancer':
            self.load_heartcancer(isIID)
        elif self.name == 'phone':
            self.load_phone(isIID)
        else:
            pass

    def load_phone(self,isIID):
        Iris_Data = pd.read_csv("phoneprice_train.csv")
        data = Iris_Data.copy()
        Y = data['price_range']

        X_data = data.drop(columns = 'price_range')
        #print(X_data)
        feature_names = X_data.columns.values.tolist()
        finx = np.arange(len(feature_names))
        feature = dict(zip(feature_names,finx))
        #print(feature)
        # xky-归一化处理将值转换为0-1之间的值
        X = (X_data - X_data.min())/(X_data.max() - X_data.min()).values
        x_train, x_test, y_train, y_test = train_test_split(X, Y, test_size = 0.4, random_state= 33)

        X_train = torch.FloatTensor(x_train.values)
        Y_train = torch.LongTensor(y_train.values)
        X_test, Y_test = torch.FloatTensor(x_test.values), torch.LongTensor(y_test.values)
        ...
        if True:
            ...
            self.train_data = X_train[order]
            self.train_label = Y_train[order]

        self.test_data = X_test
        self.test_label = Y_test

    def load_breastcancer(self,isIID):
        Iris_Data = pd.read_csv("breast_cancer.csv")
        data = Iris_Data.copy()
    ...
    def load_heartcancer(self,isIID):
        Iris_Data = pd.read_csv("heart.csv")
        data = Iris_Data.copy()
        Y = data['output']
